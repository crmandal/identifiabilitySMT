...
def regres(X, y, xtest):
    ...
    k2 = RBF(length_scale=0.1,length_scale_bounds=(1e-2,1e10))
    k3 = 1.0 * Matern(length_scale=1.0, length_scale_bounds=(1e-2,1e10), nu=1.5)
...
def classify(X, y, xtest):
    ...
    gp = GaussianProcessClassifier(kernel=kernel, \
        ...
        optimizer=optimizer)
    gp.fit(X, y)
    y_prob = gp.predict_proba(xtest) #, return_std=True)
    #y_pred = y_mean - 1.96*std_pred, y_mean + 1.96*std_pred, 
    return y_prob