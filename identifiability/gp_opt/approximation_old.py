...
def getGPRBoxes(sbox, pre_envelope, current_sbox, kdt_true, kdt_false, true_boxes, par_names, gen, extra = False):
	...
	if  gen%4 < 2:
		...
		udu = MIN_EPS_3 * 20
		# 0.05, 0.1
	du = ldu #np.random.uniform(ldu, udu, 1)[0] #0.05
...
def getNextBoxesFromGPR(sbox, pre_envelope, current_sbox, kdt_true, kdt_false, true_boxes, du, gen, extra = False):
	...
	for connected in connectedComponents:
		...
		jk += 1
		cc = sorted(cc1)
		pi = 0 #int((gen/2)%len(cc)) #0 #cc.index(pn)
		pn = cc[pi]
		#for pn in cc:
		# 				
		# pn = random.choice(cc)
		...
		'''
		# added on 18/01/2023
		c33_old = [b for b in c33]
		select_cluster_convex = getHullBoundary(c33_old)
		select_c33_1 = []
		select_c33_2 =  []
		# now divide the points in half based on its Out axis
		Out_key = out_param
		for b in c33_old:
			smap = b.get_map()
			blist = []
		...
		blist_mid = blist_sorted[int(len(blist)/2)]
		# blist_index = blist.index(blist_mid)

		for b in c33_old:
			smap = b.get_map()
			bl = ((smap[Out_key].leftBound()+smap[Out_key].rightBound())/2)
			if bl <= blist_mid:
				select_c33_1.append(b)
			else:
				select_c33_2.append(b)

		c33 = select_c33_1 if gen%2 == 1 else select_c33_2

		if len(c33) < all_param_len:
			c33 = [b for b in c33_old]
			'''
	...
	for x11 in npg_pi:
		...
		for i in range(len(param_names_sorted)):
			...
			# # changed on 11/05/2022 
			# ll = max(x1[i] - MIN_EPS_2, s_map[pn].leftBound())
			# ul = min(x1[i] + MIN_EPS_2, s_map[pn].rightBound())

			# ll = max(x1[i] - MIN_EPS_2, big_s_map[pn].leftBound())
			# ul = min(x1[i] + MIN_EPS_2, big_s_map[pn].rightBound())
...
# def getNextBoxesFromGradient(sbox, kdt_true, true_boxes, du, gen, extra = False):
	...
	for connected in connectedComponents:
		...
		c33 = select_c33_1 if gen%2 == 0 else select_c33_2
